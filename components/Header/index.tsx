...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        // Silently fail to avoid console clutter for end-users
      }
    }
  ...
  if (!mounted) return <div className="h-20 bg-black w-full" />

  return (
    <>
      {/* 1. Global System Notice Banner */}
      {notice && (
        <div className="fixed top-0 left-0 right-0 z-[60] bg-blue-600 border-b border-white/10 animate-in slide-in-from-top duration-700">
      ...
      <nav className={`fixed left-0 right-0 z-50 transition-all duration-500 ${
        ...
        scrolled 
          ? 'py-3 bg-white/90 dark:bg-black/95 backdrop-blur-xl border-b border-gray-100 dark:border-white/5' 
          : 'py-6 bg-transparent'
      }`}>
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="flex justify-between items-center">
            
            {/* Logo - Refined smaller width */}
            <Link href="/" className="group flex items-center">
              <NextImage 
                src="/hhb7Yj6zdj7QzEXWiKRVvkjJznru8eGnB5wKm2Ue.svg" 
                alt="Logo"
                width={75} 
                height={15} 
                priority
                className="h-4 w-auto dark:invert transition-transform duration-500 group-hover:scale-105"
              />
            </Link>
            ...
            <div className="hidden lg:flex items-center bg-gray-100/50 dark:bg-white/5 rounded-full px-1 py-1 border border-gray-200/50 dark:border-white/10">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="px-6 py-2 text-[11px] font-bold uppercase tracking-widest text-gray-500 hover:text-black dark:hover:text-white transition-all rounded-full hover:bg-white dark:hover:bg-white/10"
                >
                  {item.name}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              <Link
                href="/wallet"
                className="hidden sm:flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-gray-500 hover:text-blue-500 transition-colors"
              >
              ...
              {loading ? (
                ...
                <div className="relative group">
                  <button className="flex items-center gap-2 pl-2 pr-1 py-1 rounded-full bg-gray-100 dark:bg-white/5 border border-transparent hover:border-gray-200 dark:hover:border-white/10 transition-all">
                    ...
                    <div className="w-8 h-8 rounded-full bg-black dark:bg-white flex items-center justify-center relative shadow-lg">
                  ...
                  <div className="absolute right-0 top-full mt-4 w-64 bg-white dark:bg-[#0C0C0C] rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-50 overflow-hidden">
                    <div className="p-6 border-b border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/5">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {user.kycLevel === 'LEVEL_2' && <ShieldCheck className="w-3 h-3 text-blue-500" />}
                      </div>
                      <p className="text-[11px] text-gray-500 truncate lowercase">{user.email}</p>
                    </div>

                    <div className="p-3">
                      <Link href="/account" className="flex items-center justify-between px-4 py-3 text-[10px] uppercase tracking-widest font-bold text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/10 rounded-2xl transition-all">
                      ...
                      <Link href="/portfolio" className="flex items-center justify-between px-4 py-3 text-[10px] uppercase tracking-widest font-bold text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/10 rounded-2xl transition-all">
                        <div className="flex items-center gap-3">
                ...
                <Link 
                  href="/login" 
                  className="bg-black dark:bg-white text-white dark:text-black px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest hover:opacity-80 transition-opacity"
                >
                  Sign In
                </Link>
              )}
