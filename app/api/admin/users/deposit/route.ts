...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Independent Wallet Deposit (Stealth terminology)
    ...
    if (!isNaN(depositAmount) && depositAmount > 0) {
      ...
      user.wallet.transactions.push({
        ...
        status: "completed",
        date: new Date(),
        // Professional fallback if admin leaves description blank
    ...
    // Always keep Total Value synced: (Principal + Growth)
